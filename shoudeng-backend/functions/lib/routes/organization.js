                            ...
                            payload: {
                                aps: {
                                    sound: severity === "critical"
                                        ? "sos_alert.caf"
                                        : "default",
                                    "interruption-level": severity === "critical"
                                        ? "critical"
                                        : "active",
                                },
                            },
 ...
 */
router.post("/roll-call/respond", async (req, res) => {
    const uid = req.uid;
 ...
 */
router.get("/roll-call/:alertId", async (req, res) => {
    const uid = req.uid;
    ...
    try {
        ...
        res.json({
            ...
            summary: {
                ...
                totalAlerts,
                totalSOSEvents,
                avgCheckInsPerMember: memberIds.length > 0
                    ? Math.round(totalCheckIns / memberIds.length)
                    : 0,
            },
            generatedAt: new Date().toISOString(),
            disclaimer: "此报告由守灯安全系统自动生成，仅供合规审计参考。",
        });
    }
    catch (error) {
